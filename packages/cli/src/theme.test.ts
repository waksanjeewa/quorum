...
describe("terminal launch logo", () => {
  it("combines the compact logo-v2 mark with the correct ASCII QUORUM word", () => {
    ...
    expect(QUORUM_TERMINAL_LOCKUP[0].trim()).toBe("●");
    expect(QUORUM_TERMINAL_LOCKUP[1]).toContain(QUORUM_ASCII_WORD[0]);
    expect(QUORUM_TERMINAL_LOCKUP[5]).toContain(QUORUM_ASCII_WORD[4]);
    expect(QUORUM_TERMINAL_LOCKUP[6]).toContain("many models, working together");
  });
});
