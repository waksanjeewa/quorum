...
import { buildConfigYaml, runFrugalSetup, runSetup } from "./setup.js";
...
describe("buildConfigYaml", () => {
  ...
  it("frugal mode can use an explicit free/paid model selection", () => {
    const yaml = buildConfigYaml(
      ["claude", "codex", "openrouter/deepseek/deepseek-chat:free", "ollama/llama3"],
      { openrouter: { baseUrl: "https://openrouter.ai/api/v1", keyEnv: "OPENROUTER_API_KEY" } },
      { frugal: true, frugalFreeModels: ["openrouter/deepseek/deepseek-chat:free"], frugalPaidModels: ["codex"] },
    );
    const cfg = parseSessionConfig(parseYaml(yaml));
    expect(cfg.seats.proposer?.chain).toEqual(["openrouter/deepseek/deepseek-chat:free", "codex"]);
    expect(cfg.seats.critic?.chain).toEqual(["codex", "openrouter/deepseek/deepseek-chat:free"]);
    expect(cfg.seats.arbiter?.chain).toEqual(["codex", "openrouter/deepseek/deepseek-chat:free"]);
  });
...
describe("runSetup (interactive flow)", () => {
  ...
  it("/frugal setup lets the user choose free draft and paid verifier subsets", async () => {
    ...
    expect(cfg.seats.critic?.chain).toEqual(["codex", "ollama/llama3"]);
    expect(cfg.seats.arbiter?.chain).toEqual(["codex", "ollama/llama3"]);
  });
});
